...
'######################################################################  TESTING SQL STATEMENT
'######################################################################  TESTING SQL STATEMENT
'strsql2="SELECT COMBINED.dataset_id, COMBINED.RecordID, COMBINED.id, COMBINED.Abstract, COMBINED.Title FROM (File_Attribute_table INNER JOIN (File_Attribute_status INNER JOIN COMBINED ON File_Attribute_status.dataset_id = COMBINED.dataset_id) ON (File_Attribute_status.dataset_id = File_Attribute_table.dataset_id) AND (File_Attribute_table.dataset_id = COMBINED.dataset_id)) INNER JOIN File_Attribute_Column_Descriptions ON File_Attribute_table.File_Attribute_Type = File_Attribute_Column_Descriptions.File_Attribute_Type;"
...
'2013-05-03 - INNER JOIN NOT QUITE RIGHT.  I SHOULD ONLY SEE RECORDS WHERE dataset_id's are equal.  Instead I get all of them.  I could specify dataset_id not null but I should not have to on an inner join.
...
Response.write "'************************************************ <br /> " 
Response.write "'************************************************ <br /> " 
Response.write "'BEGIN LOOP TO WRITE ccccc SPATIAL AND NON SPATIAL DATA TO EML FILES  USING TABLE 'COMBINED'<br />"
Response.write "'************************************************ <br /> " 
Response.write "'************************************************ <br /> " 
Response.write "'************************************************ <br /> " 
...
do while not xrs.EOF 'and emergencystop<2
	...
	fname=MrecordID
	response.write "Is MrecordID equal to xrs(recordID)??* "
	response.write MrecordID=xrs("RecordID")  	'HERE WE Are not looking at the value.  We are looking at the comparison t/f.
	response.write " * <br />"
	response.write "Dataset_id= " & xrs("dataset_id") & " <br/>"
...
'	objTextFile.WriteLine "<access authSystem=""knb"" order=""allowFirst"" scope=""document"">"
'	objTextFile.WriteLine "<allow>"


response.write xrs("File_Attribute_Type")
...
'</access>
	...
	'###################################################################################################
		...
		if len(trim(xrs("ThemeKeywords")))>0 then
			...
			testword=trim(xrs("ThemeKeywords"))
			...
			do while stoppy<100 'Stoppy keeps the loop from going indefinitely
				...
				'WRITE IT TO FILE
				objTextFile.WriteLine "<keyword keywordType=""theme"">"& thekeyword &"</keyword>"
		...
		if len(trim(xrs("PlaceKeywords")))>0 then
			newword="anything"
			stoppy=0 'Stoppy keeps the loop from going indefinitely
			testword=trim(xrs("placeKeywords"))
			'testword="peter, paul, mary, george, ralphie"
			do while stoppy<100 'Stoppy keeps the loop from going indefinitely
				stoppy=stoppy+1
				'response.write "<br /> ####################### <br />"
				'response.write vbcrlf
				'response.write "stoopy= " & stoppy & "***"
				'response.write " testword is *" & testword  & "***"
				testword=replace(testword, ",  ", ", ")
				origlen=len(testword)
				'response.write " length original = " & origlen & "***"
				...
				'response.write "length of newword=" & len(newword)
				'WRITE IT TO FILE
				objTextFile.WriteLine "<keyword keywordType=""place"">"& thekeyword &"</keyword>"
				if instr(testword,",")=0 then 'no more commas, must be on the last keyword.  Out we go.
					response.write "    EXITING DO <br />"
					exit do
				end if
			...
			objTextFile.WriteLine t&t&t& "<keyword keywordType=""place"">Not Available</keyword>"
		end if
		objTextFile.WriteLine t& "</keywordSet>"
